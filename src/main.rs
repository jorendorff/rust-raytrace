...
fn hit_sphere(center: Vec3, radius: f32, r: Ray) -> Option<Vec3> {
...
fn color(r: Ray) -> Vec3 {
    ...
    if let Some(n) = hit_sphere(Vec3(0.0, 0.0, -1.0), 0.5, r) {
        let d = n.to_unit_vector().dot(unit_direction).abs();  // directness
